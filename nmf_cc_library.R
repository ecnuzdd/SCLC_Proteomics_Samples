...
get_n_year_surv_rate = function(surv_model, surv_df, n = 5){
  ...
  n_year_surv_rate_df$count = surv_model$n
  order_index_desc = order(n_year_surv_rate_df$surv, decreasing = T)
  n_year_surv_rate_df = n_year_surv_rate_df[order_index_desc,]
  row.names(n_year_surv_rate_df) = n_year_surv_rate_df$subtype = 1:length(cluster_num)
  n_year_surv_rate_df = n_year_surv_rate_df[, c('time', 'surv', 'lower', 'upper', 'cluster', 'subtype', 'count')]
  return(n_year_surv_rate_df)
}
